...
public class AttributeLoadTests extends ThreadPooledTestSuite {
    ...
    private static final int[] BATCH_SIZES = new int[]{10, 100, 1000};
    ...
    /**
     * Load test with 4KB max page size and 1M attributes.
     */
    @Test
    public void testLoad4K1M() {
        final int pageSize = 4 * 1024;
        final int count = 1000 * 1000;
        for (int batchSize : BATCH_SIZES) {
            testLoad(pageSize, count, batchSize, true);
        }
    }

    /**
     * Load test with 4KB max page size and 10M attributes.
     */
    @Test
    public void testLoad4K10M() {
        final int pageSize = 4 * 1024;
        final int count = 10 * 1000 * 1000;
        for (int batchSize : BATCH_SIZES) {
            testLoad(pageSize, count, batchSize, false);
        }
    }

    /**
     * Load test with 8KB max page size and 1M attributes.
     */
    @Test
    public void testLoad8K1M() {
    ...
    /**
     * Load test with 16KB max page size and 1M attributes.
     */
    @Test
    public void testLoad16K1M() {
    ...
    private void testLoad(int pageSize, int attributeCount, int batchSize, boolean testGet) {
        System.out.println(String.format("Started: PageSize = %s, Count = %s, Batch = %s ...", pageSize, attributeCount, batchSize));

        String segmentName = String.format("page_%s_count_%s_batch_%s/segment", pageSize, attributeCount, batchSize);
        ...
        executeInBatches(attributeCount, batchSize, (count, batch) -> {
        ...
        double excess = (double) (actualDataSize - theoreticalDataSize) / theoreticalDataSize * 100;
        ...
        if (testGet) {
            ...
            executeInBatches(attributeCount, batchSize, (count, batch) -> idx.get(batch.keySet(), TIMEOUT).join());
    ...
    private void executeInBatches(int attributeCount, int batchSize, BiConsumer<Integer, Map<UUID, Long>> processBatch) {
        int count = 0;
        int batchId = 0;
        while (count < attributeCount) {
            ...
            for (int indexInBatch = 0; indexInBatch < bs; indexInBatch++) {
                val key = getKey(batchId, indexInBatch);
    ...
    private UUID getKey(int batchId, int indexInBatch) {
        return new UUID(batchId, indexInBatch);
    }
