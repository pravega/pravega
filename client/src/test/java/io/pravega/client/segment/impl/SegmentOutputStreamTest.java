...
public class SegmentOutputStreamTest extends ThreadPooledTestSuite {
    ...
    public void testClose() throws ConnectionFailedException, SegmentSealedException {
        ...
        verify(connection, Mockito.atMost(1)).send(new WireCommands.CloseAppend(cid));
    ...
    public void testFailOnClose() throws ConnectionFailedException {
        ...
        }).doNothing().when(connection).send(new WireCommands.CloseAppend(cid));
        doAnswer(new Answer<Void>() {
            @Override
            public Void answer(InvocationOnMock invocation) throws Throwable {
                CompletedCallback callback = (CompletedCallback) invocation.getArgument(1);
        ...
        inOrder.verify(connection).send(new WireCommands.CloseAppend(cid));
