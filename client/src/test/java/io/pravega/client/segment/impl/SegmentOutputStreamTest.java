...
public class SegmentOutputStreamTest extends ThreadPooledTestSuite {
    ...
    public void testConnectAndFailedSetupAppendDueToTruncation() throws Exception {
        ...
        cf.provideConnection(uri, connection);
        @Cleanup
        SegmentOutputStreamImpl output = new SegmentOutputStreamImpl(SEGMENT, false, controller, cf, cid, resendToSuccessorsCallback,
                                                                     RETRY_SCHEDULE, "");
        output.reconnect();
        verify(connection).send(new SetupAppend(output.getRequestId(), cid, SEGMENT, ""));
    ...
    public void testFailDuringFlush() throws Exception {
        ...
        InOrder order = Mockito.inOrder(connection);
        @Cleanup
        SegmentOutputStreamImpl output = new SegmentOutputStreamImpl(SEGMENT, false, controller, cf, cid,
    ...
    public void testExceptionSealedCallback() throws Exception {
        ...
        SegmentOutputStreamImpl output = new SegmentOutputStreamImpl(SEGMENT, false, controller, cf, cid, exceptionCallback,
                                                                     RETRY_SCHEDULE, "");
        output.reconnect();
        verify(connection).send(new SetupAppend(output.getRequestId(), cid, SEGMENT, ""));
        cf.getProcessor(uri).appendSetup(new AppendSetup(output.getRequestId(), SEGMENT, cid, 0));
    ...
    public void testFlushDuringTransactionAbort() throws Exception {
        ...
        SegmentOutputStreamImpl output = new SegmentOutputStreamImpl(TXN_SEGMENT, false, controller, cf, cid, segmentSealedCallback,
                                                                     RETRY_SCHEDULE, "");
        output.reconnect();
    ...
    @Test(timeout = 10000)
    public void testConnectAndSendWithConnectionPooling() throws SegmentSealedException, ConnectionFailedException {
        UUID cid = UUID.randomUUID();
        PravegaNodeUri uri = new PravegaNodeUri("endpoint", SERVICE_PORT);
        MockConnectionFactoryImpl cf = new MockConnectionFactoryImpl();
        cf.setExecutor(executorService());
