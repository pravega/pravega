...
public class ClientFactoryImpl implements ClientFactory, EventStreamClientFactory, SynchronizerClientFactory {
    ...
    private void verifyStartAndEndStreamCuts(final StreamCut startStreamCut, final StreamCut endStreamCut) {
        ...
        //* verify both stream cut satisfy the "cover full key space" and "no key space overlap" in each of the streamcuts
        Preconditions.checkArgument(controller.isStreamCutValid(startStreamCut).equals(true),
                "startStreamCut key space violation.");
        Preconditions.checkArgument(controller.isStreamCutValid(endStreamCut).equals(true),
                "endStreamCut key space violation.");

        //* check if the offsets are legal
