...
/**
 * A stream can be thought of as an unlimited sequence of events.
 * A stream can be written to or read from.
 * A stream is:
 * Append only (Events in it are immutable once written)
 * Unlimited (There are no limitations in size or time to how many events can go into a stream)
