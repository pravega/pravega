...
public class ReaderGroupStateManager {
    ...
    void initializeReader() {
        ...
        sync.updateState(state -> {
            if (state.getSegments(readerId) == null) {
                return Collections.singletonList(new ReaderGroupState.AddReader(readerId));
            } else {
    ...
    static void readerShutdown(String readerId, Position lastPosition, StateSynchronizer<ReaderGroupState> sync) {
        sync.updateState(state -> {
            ...
            return Collections.singletonList(new ReaderGroupState.RemoveReader(readerId, lastPosition == null ? null : lastPosition.asImpl()));
        });
    }
    ...
    void handleEndOfSegment(Segment segmentCompleted) throws ReinitializationRequiredException {
        ...
        sync.updateState(state -> {
            ...
            return Collections.singletonList(new ReaderGroupState.SegmentCompleted(readerId, segmentCompleted, successors.getSegmentToPredecessor()));
        });
    ...
    boolean releaseSegment(Segment segment, long lastOffset, long timeLag) throws ReinitializationRequiredException {
        sync.updateState(state -> {
            ...
            List<ReaderGroupState.ReaderGroupStateUpdate> result = new ArrayList<>(2);
            result.add(new ReaderGroupState.ReleaseSegment(readerId, segment, lastOffset));
            result.add(new ReaderGroupState.UpdateDistanceToTail(readerId, timeLag));
            return result;
        });
    ...
    private Map<Segment, Long> acquireSegment(long timeLag) throws ReinitializationRequiredException {
        ...
        sync.updateState(state -> {
            ...
            List<ReaderGroupState.ReaderGroupStateUpdate> updates = new ArrayList<>(toAcquire);
            ...
            for (int i = 0; i < toAcquire; i++) {
                ...
                updates.add(new ReaderGroupState.AcquireSegment(readerId, segment.getKey()));
            }
            updates.add(new ReaderGroupState.UpdateDistanceToTail(readerId, timeLag));
            result.set(acquired);
            return updates;
        });
        if (reinitRequired.get()) {
    ...
    void checkpoint(String checkpointName, PositionInternal lastPosition) throws ReinitializationRequiredException {
        ...
        sync.updateState(state -> {
            ...
            return Collections.singletonList(new ReaderGroupState.CheckpointReader(checkpointName, readerId, lastPosition.getOwnedSegmentsWithOffsets()));
        });
        if (reinitRequired.get()) {
            throw new ReinitializationRequiredException();
        }
