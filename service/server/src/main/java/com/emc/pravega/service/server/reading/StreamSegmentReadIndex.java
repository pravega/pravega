...
class StreamSegmentReadIndex implements CacheManager.Client, AutoCloseable {
    ...
    public long updateGenerations(int currentGeneration, int oldestGeneration) {
        ...
        synchronized (this.lock) {
            this.indexEntries.values().forEach(entry -> {
                ...
                boolean canRemove = !(entry instanceof RedirectReadIndexEntry) && entry.getLastStreamSegmentOffset()
                        <= this.metadata.getStorageLength() && entry.getGeneration() < oldestGeneration;
                if (canRemove) {
                    toRemove.add(entry);
                }
            });
    ...
    public void exitRecoveryMode(SegmentMetadata newMetadata) {
        ...
        Exceptions.checkArgument(newMetadata.getId() == this.metadata.getId(), "newMetadata",
                "New Metadata " + "StreamSegmentId is different from existing one.");
        ...
        Exceptions.checkArgument(newMetadata.isSealed() == this.metadata.isSealed(), "newMetadata",
                "New Metadata " + "Sealed Flag is different from existing one.");
        Exceptions.checkArgument(newMetadata.isMerged() == this.metadata.isMerged(), "newMetadata",
                "New Metadata " + "Merged Flag is different from existing one.");
        Exceptions.checkArgument(newMetadata.isDeleted() == this.metadata.isDeleted(), "newMetadata",
                "New Metadata " + "Deletion Flag is different from existing one.");
    ...
    public void append(long offset, byte[] data) {
        ...
        Preconditions.checkState(!isMerged(),
                "StreamSegment has been merged into a different one. Cannot append more" + " ReadIndex entries.");
        ...
        Exceptions.checkArgument(endOffset <= durableLogLength, "offset",
                "The given range of bytes (%d-%d) is beyond" + " the StreamSegment Durable Log Length (%d).", offset,
                endOffset, durableLogLength);
    ...
    public void beginMerge(long offset, StreamSegmentReadIndex sourceStreamSegmentIndex) {
        ...
        Exceptions.checkNotClosed(this.closed, this);
        Preconditions.checkState(this.metadata.getParentId() == ContainerMetadata.NO_STREAM_SEGMENT_ID,
                "Cannot merge" + " a StreamSegment into a child StreamSegment.");
        Exceptions.checkArgument(!sourceStreamSegmentIndex.isMerged(), "sourceStreamSegmentIndex",
                "Given " + "StreamSegmentReadIndex is already merged.");
        ...
        Exceptions.checkArgument(sourceMetadata.isSealed(), "sourceStreamSegmentIndex",
                "Given StreamSegmentReadIndex" + " refers to a StreamSegment that is not sealed.");
    ...
    public void completeMerge(long sourceSegmentStreamId) {
        ...
        synchronized (this.lock) {
            ...
            assert treeEntry != null && (treeEntry instanceof RedirectReadIndexEntry) : String.format(
                    "mergeOffsets " + "points to a ReadIndexEntry that does not exist or is of the wrong type. " +
                            "sourceStreamSegmentId = " + "%d, offset = %d, treeEntry = %s.",
                    sourceSegmentStreamId, endOffset, treeEntry);
    ...
    private void appendEntry(ReadIndexEntry entry) {
        log.debug("{}: Append (Offset = {}, Length = {}).", this.traceObjectId, entry.getStreamSegmentOffset(),
                entry.getLength());

        synchronized (this.lock) {
            Exceptions.checkArgument(
                    this.lastAppendedOffset < 0 || entry.getStreamSegmentOffset() == this.lastAppendedOffset + 1,
                    "entry",
                    "The given range of bytes (%d-%d) does not start right after " + "the last appended range (%d).",
                    entry.getStreamSegmentOffset(), entry.getLastStreamSegmentOffset(), this.lastAppendedOffset);
            ...
            assert oldEntry == null : String.format(
                    "Added a new entry in the ReadIndex that overrode an existing " + "element. New = %s, Old = %s.",
                    entry, oldEntry);
        ...
        synchronized (this.lock) {
            Exceptions.checkArgument(entry.getLastStreamSegmentOffset() < this.metadata.getStorageLength(), "entry",
                    "The given range of bytes (%d-%d) does not correspond to the StreamSegment range that is in " +
                            "Storage (%d).",
                    entry.getStreamSegmentOffset(), entry.getLastStreamSegmentOffset(),
    ...
    public void triggerFutureReads() {
        ...
        for (FutureReadResultEntry r : futureReads) {
            ...
            log.trace("{}: triggerFutureReads (Offset = {}, Type = {}).", this.traceObjectId,
                    r.getStreamSegmentOffset(), entry.getType());
            if (entry.getType() == ReadResultEntryType.EndOfStreamSegment) {
                ...
                // message.
                r.fail(new StreamSegmentSealedException(String.format(
                        "StreamSegment has been sealed at offset %d. " + "There can be no more reads beyond this " +
                                "offset.",
    ...
    public ReadResult read(long startOffset, int maxLength, Duration timeout) {
        ...
        Exceptions.checkArgument(canReadAtOffset(startOffset), "startOffset",
                "StreamSegment is sealed and " + "startOffset is beyond the last offset of the StreamSegment.");
    ...
    private ReadResultEntryBase getFirstReadResultEntry(long resultStartOffset, int maxLength) {
            ...
            if (this.indexEntries.size() == 0) {
                ...
                if (treeEntry == null) {
                    ...
                    if (resultStartOffset < currentEntry.getStreamSegmentOffset()) {
                        ...
                        // of the gap).
                        int readLength = (int) Math.min(maxLength,
                                currentEntry.getStreamSegmentOffset() - resultStartOffset);
                        result = createStorageRead(resultStartOffset, readLength);
                    } else if (currentEntry instanceof RedirectReadIndexEntry) {
                        result = getRedirectedReadResultEntry(resultStartOffset, maxLength,
                                (RedirectReadIndexEntry) currentEntry);
                    } else {
        ...
        assert result != null : String.format(
                "Reached the end of getFirstReadResultEntry(id=%d, offset=%d, " + "length=%d) with no plausible " +
                        "result in sight. This means we missed a case.",
                this.metadata.getId(), resultStartOffset, maxLength);
        return result;
    }
    ...
    private ReadResultEntryBase createMemoryRead(ReadIndexEntry entry, long streamSegmentOffset, int maxLength) {
        assert streamSegmentOffset >= entry.getStreamSegmentOffset() : String.format(
                "streamSegmentOffset{%d} < entry" + ".getStreamSegmentOffset{%d}", streamSegmentOffset,
        ...
        assert length > 0 : String.format(
                "length{%d} <= 0. streamSegmentOffset = %d, maxLength = %d, entry.offset = " + "%d, entry.length = %d",
                length, streamSegmentOffset, maxLength, entry.getStreamSegmentOffset(), entry.getLength());
    ...
    private int getReadAlignedLength(long offset, int readLength) {
        ...
        return Math.min(readLength, Math.max(this.config.getStorageReadMinLength(),
                this.config.getStorageReadMaxLength() - lengthSinceLastMultiple));
    }
    ...
    private List<ReadIndexEntry> getAllEntries(long offsetAdjustment) {
        Exceptions.checkArgument(offsetAdjustment >= 0, "offsetAdjustment",
                "offsetAdjustment must be a non-negative " + "number.");
        ...
        synchronized (this.lock) {
            ...
            for (ReadIndexEntry entry : this.indexEntries.values()) {
                if (!(entry instanceof RedirectReadIndexEntry)) {
                    MergedReadIndexEntry me = new MergedReadIndexEntry(
                            entry.getStreamSegmentOffset() + offsetAdjustment, entry.getLength(), this.metadata.getId(),
                            entry.getStreamSegmentOffset());
                    result.add(me);
                }
            }
