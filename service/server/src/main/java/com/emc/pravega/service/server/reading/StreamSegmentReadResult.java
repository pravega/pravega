...
class StreamSegmentReadResult implements ReadResult {
    ...
    public ReadResultEntry next() {
        ...
        if (entry == null) {
            ...
            if (entry.getType() == ReadResultEntryType.EndOfStreamSegment) {
                ...
                // If we don't close the ReadResult, hasNext() will erroneously return true and next() will have
                // undefined behavior.
                this.lastEntryFuture = null;
                ...
                // and before we got the lastEntryFuture. If this happened, throw the exception and don't return
                // anything.
                Exceptions.checkNotClosed(this.closed, this);
            }
        }
    ...
    /**
     * Defines a Function that given a startOffset (long) and remainingLength (int), returns the next entry to be
     * consumed (ReadResultEntry).
     */
    @FunctionalInterface
