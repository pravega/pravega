...
public class StreamSegmentReadResultTests {
    ...
    public void testNextFullyConsumed() {
        ...
        for (int i = 0; i < MAX_RESULT_LENGTH; i += READ_ITEM_LENGTH) {
            ...
            Assert.assertTrue("hasNext() returned false even though we haven't consumed the entire result.", r
                    .hasNext());
            ...
            Assert.assertEquals("getStreamSegmentStartOffset changed while iterating.", START_OFFSET, r
                    .getStreamSegmentStartOffset());
            Assert.assertEquals("getMaxResultLength changed while iterating.", MAX_RESULT_LENGTH, r
                    .getMaxResultLength());
            Assert.assertEquals("Unexpected value from getConsumedLength after returning a value but before " +
                    "completing result future.", expectedConsumedLength, r.getConsumedLength());
            ...
            Assert.assertEquals("Unexpected value from getConsumedLength after returning a value and completing " +
                    "result future.", expectedConsumedLength, r.getConsumedLength());
        }

        // Verify we have reached the end.
        Assert.assertEquals("Unexpected state of the StreamSegmentReadResult when consuming the entire result.", r
                .getMaxResultLength(), r.getConsumedLength());
    ...
    public void testNextEndOfStreamSegment() {
        ...
        AssertExtensions.assertLessThan("Unexpected state of the StreamSegmentReadResult when consuming half of the " +
                "result.", r.getMaxResultLength(), r.getConsumedLength());
        ...
        Assert.assertFalse("hasNext() did not return false when reaching the end of a sealed StreamSegment.", r
                .hasNext());
        resultEntry = r.next();
    ...
    public void testClose() {
        ...
        r.close();
        Assert.assertTrue("Already returned result future is not canceled after closing the ReadResult.", resultEntry
                .getContent().isCancelled());
