...
public class MonitorFactory {
    ...
    public static SegmentTrafficMonitor createMonitor(MonitorType monitorType) {
        if (monitorType.equals(MonitorType.ThresholdMonitor)) {
            ...
            ClientFactory clientFactory = new ClientFactoryImpl(SCOPE, URI.create(String.format("tcp://%s:%d", CONTROLLER_ADDR, CONTROLLER_PORT)));

            return ThresholdMonitor.getMonitor(clientFactory);
        }
        return null;
    }
}
