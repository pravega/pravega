...
The below API can be used to **reset an existing Reader Group with a new Reader Group** configuration instead creating a
Reader Group.
```
/*
...
Enable `getStreamInfo(String scopeName, String streamName)` API on `StreamManager`. By enabling the `getStreamInfo()` API on `StreamManager` and deprecates the `getStreamInfo()` API inside the `BatchClient`. This ensure the `getStreamInfo()` API can be used outside the `BatchClient`.
