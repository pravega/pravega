...
and Writers (see Working with Reader and Writer) before continuing reading this page.

## Definition

StreamCut represents a consistent position in the stream. It essentially contains a set of segments
...
stream(with the newest event) is an ever changing one. This is similar with the stream cut representing
the head of the stream(with the oldest event) as the stream retention policy could truncate the stream.
StreamCut.UNBOUNDED is used to represent such a position in the stream and the user can use it to
specify this every changing stream position.
...
io.pravega.client.stream.impl.ReaderGroupImpl.getStreamCuts


## StreamCuts with BatchClient.

-- pre requisite for using this.

-- what is a streamcut?
-- usage pattern in Streaming client.
    -- Just starting streamCut.
    -- Just sstarting and end streamCut.
    -- in case of stream truncation.

-- usage pattern in Batch client.
    -- fetch stream info.
    -- fetch all the segments in parallel.
