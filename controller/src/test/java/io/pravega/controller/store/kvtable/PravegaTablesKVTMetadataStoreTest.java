...
public class PravegaTablesKVTMetadataStoreTest extends KVTableMetadataStoreTest {
    ...
    public void testInvalidOperation() throws Exception {
        ...
        KeyValueTableConfiguration configuration = KeyValueTableConfiguration.builder().partitionCount(2).build();
        byte[] newUUID = store.newScope(scope).newId();
        store.createEntryForKVTable(scope, kvtable1, newUUID, executor);
        store.createKeyValueTable(scope, kvtable1, configuration, System.currentTimeMillis(), null, executor).get();
