...
public abstract class StreamMetadataStoreTest {
    ...
    public void txnOrderTest() throws Exception {
        ...
        List<UUID> ordered = orderedRecords.stream().map(x -> x.getId()).collect(Collectors.toList());
        ...
        ordered = orderedRecords.stream().map(x -> x.getId()).collect(Collectors.toList());
        assertEquals(1, ordered.size());
        assertEquals(tx00, ordered.get(0));
    ...
    public void txnCommitBatchLimitOrderTest() throws Exception {
        ...
        while (!txns.isEmpty()) {
            int limit = 5;
            List<VersionedTransactionData> orderedRecords = streamObj.getOrderedCommittingTxnInLowestEpoch(limit, context).join();
            List<UUID> ordered = orderedRecords.stream().map(x -> x.getId()).collect(Collectors.toList());
