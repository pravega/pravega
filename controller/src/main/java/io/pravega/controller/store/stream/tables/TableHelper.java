...
public class TableHelper {
    ...
    public static boolean isScaleInputValid(final List<Integer> segmentsToSeal,
        ...
        if (newRanges.isEmpty()) {
            // Stream is being sealed;
            return reduce(oldRanges).get(0).equals(new AbstractMap.SimpleEntry<>(0.0, 1.0));
        } else {
            // It is a normal scale operation,
            // in which case the new ranges should match key ranges of segments to be sealed.
