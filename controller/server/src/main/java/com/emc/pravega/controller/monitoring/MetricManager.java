...
public abstract class MetricManager<V1, H1 extends History<HostMetric, V1>, V2, H2 extends History<StreamMetric, V2>> {
    ...
    public void addStream(final StreamData streamData) {
        ...
        final ScaleActionProcessor actionProcessor = new ScaleActionProcessor(scaleActionQueue, streamMetadataTasks, stream, scope);

        final StreamMonitor<V2, H2> monitor = getStreamMonitor(scaleActionQueue, actionProcessor, stream, scope, policy, activeSegments);
