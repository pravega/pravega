...
import com.emc.pravega.controller.monitoring.datasets.HostSet;
 ...
 */
@Data
public class MonitoringMain implements Observer, Runnable {
    ...
    public MonitoringMain(final StreamMetadataStore streamStore,
    ...
    private void handleStreamChange(StreamNotification notification) {
        ...
        switch (notification.getNotificationType()) {
            case Add:
                metricManager.addStream(stream);
                break;
            case Alter:
                metricManager.policyUpdate(stream);
                break;
            case Scale:
                metricManager.scaled(stream);
                break;
        }
    }
    ...
    private void handleHostChange(HostNotification notification) {
        ...
        switch (notification.getNotificationType()) {
            case Add:
                metricManager.addHost(host);
                break;
            case Remove:
                metricManager.removeHost(host);
                break;
        }
    }
    ...
    public void run() {
        ...
        while (true) {
            ...
            metricManager.readMetrics(metrics);
        }
    }
}
