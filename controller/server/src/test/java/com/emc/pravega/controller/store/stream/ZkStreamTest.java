...
public class ZkStreamTest {
    ...
    public void testTransaction() throws Exception {
        ...
        VersionedTransactionData tx = store.createTransaction(scopeName, streamName, 10000, 600000, 30000).get();
        ...
        VersionedTransactionData tx2 = store.createTransaction(scopeName, streamName, 10000, 600000, 30000).get();
        y = store.getAllActiveTx().get();

        assert y.size() == 2;

        store.sealTransaction(scopeName, streamName, tx.getId(), true, Optional.<Integer>empty()).get();
        assert store.transactionStatus(scopeName, streamName, tx.getId()).get().equals(TxnStatus.COMMITTING);

        CompletableFuture<TxnStatus> f1 = store.commitTransaction(scopeName, streamName, tx.getId());

        store.sealTransaction(scopeName, streamName, tx2.getId(), false, Optional.<Integer>empty()).get();
        assert store.transactionStatus(scopeName, streamName, tx2.getId()).get().equals(TxnStatus.ABORTING);

        CompletableFuture<TxnStatus> f2 = store.abortTransaction(scopeName, streamName, tx2.getId());

        CompletableFuture.allOf(f1, f2).get();

        assert store.transactionStatus(scopeName, streamName, tx.getId()).get().equals(TxnStatus.COMMITTED);
        assert store.transactionStatus(scopeName, streamName, tx2.getId()).get().equals(TxnStatus.ABORTED);
