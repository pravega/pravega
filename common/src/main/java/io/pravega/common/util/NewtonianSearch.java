...
public class NewtonianSearch {
    ...
    public static <T, R extends Comparable<R>> int newtonianSearch(Function<T, R> valueExtractor, R targetValue,
        ...
        while (low <= high) {
            int mid = interpolate(low, high, valueExtractor.apply((T) createInteger(low)),
                    valueExtractor.apply((T) createInteger(high)), targetValue);
    ...
    private static Integer createInteger(int value) {
        return value;
    }
