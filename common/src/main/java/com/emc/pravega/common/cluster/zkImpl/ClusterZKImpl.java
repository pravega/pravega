...
public class ClusterZKImpl implements Cluster, AutoCloseable {
    ...
    @Override
    public void registerNode(String clusterName, NodeType nodeType, EndPoint endPoint) {
        String path = new StringBuffer(PATH_CLUSTER).append(clusterName).append("/").append(endPoint.getHost()).toString();
        PersistentNode node = new PersistentNode(client, CreateMode.EPHEMERAL, false, path, endPoint.toString().getBytes());
        node.start(); //start creation of ephemeral node in background.
        entryMap.put(endPoint.getHost(), node);
    }

    @Override
    public void deregisterNode(String clusterName, NodeType nodeType, EndPoint endPoint) {
