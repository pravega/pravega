...
public interface EventRead<T> {
     ...
     * {@link ReaderGroup} this reader could have acquired more segments of the stream from
     * another reader that was behind) If all of the readers in a group have passed a certain
     ...
     * readers is about to or needs to occur. 
     * 
     * For a reader that is part of a {@link ReaderGroup} this means the next call to
     * {@link EventStreamReader#readNextEvent(long)} this reader may receive events from a different set of
     * RoutingKeys. Once this reader calls {@link EventStreamReader#readNextEvent(long)} some of the
     * routing keys it was previously reading may be taken over by another reader.
     * This is relevant if the reader processes events in a way that involves holding state in
     ...
     * 
     * For a reader that is not part of a {@link ReaderGroup} and is rebalanced manually, 
     * this means the application should call {@link RebalancerUtils#rebalance()} with the positions
     * of its readers and recreate new readers from the resulting positions. 
     * 
